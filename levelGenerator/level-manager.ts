...
export class LevelManager extends Behavior {
    ...
    private* LoadScene(){
        ...
        console.log(this._level + " is level");
        let environment;
        if (this._environments.length === 0) {
            throw new Error("no _environments are given");
        }
        for (let i = 0; i < this._environments.length; i++) {
            if (this._level >= this._environments[i].minLevels && this._level <= this._environments[i].maxLevels){
        ...
        console.log(environment.entity.name);
